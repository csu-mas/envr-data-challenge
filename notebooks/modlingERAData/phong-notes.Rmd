...
JD has done some great work and I will see if I can continue in his direction. I will explore the `PREC` variable and perform some analysis with the goal in mind to reduce the overall dimensions. My thought here is to simply look for observations that standout - outliers. This will then allow us to perform a much more through analysis on a particuliar region in time. 
...
I will demonstrate the function in action.
...
ggplot() +
  geom_raster(data = maxt.long[maxt.long$year=="1988",],
              aes(x=lon-360,y=lat,fill=mean.maxt), interpolate = TRUE) +
  scale_fill_distiller("Dark2") +
  geom_contour(data = prec.long[prec.long$year=="1988",], 
               aes(x=lon-360,y=lat,z=total.prec), color="red") +
  conus.map() +
  theme_bw()
```
...
In the next block of code I will employ hierarchical cluster analysis to simply group our anomalies into similiar clusters. This again continues with the goal subsetting the dataset and focusing on potential areas of interest.
```{r}
...
## Visualize regions of unusual levels of annual precipitation
ggplot(outliers) +
  geom_point(aes(x = lon - 360, y = lat, color = factor(cluster))) +
  facet_wrap(~time) +
  labs(x = "Longitude", 
       y = "Latitude", 
       color = "Cluster", 
...
## Previous map is kind of hard to see. Lets view 2004 in detail.

...
prec.month.ts.sd.long <- prec.month.ts %>% melt() %>% rename(lon = Var1, lat = Var2, time = Var3, sd.residual = value)
...
ggplot(data = outliers.2012, aes(x = lon - 360, y = lat)) +
  ...
  geom_label(data = centers, aes(x = lon - 360, y = lat, label = 1:5), size = 6, fontface="bold") +
...
The map is hard to see when outputed in this document, so I saved is as anomalies-plot.jpg in this file directory.
